...
router.get("/on-going", protect, async (req, res) => {
    const list = await PreEmployment.find({
        ...
        "id name aadhar_no physical_parameters.status opthalmic_examination.status",
    );
...
router.get("/vitals/:id", protect, async (req, res) => {
    try {
        const vitals = await PreEmployment.findOne({
            id: req.params.id,
        }).select("id physical_parameters");
        res.json(vitals);
    } catch (err) {
        console.log(err);
    }
...
router.get("/vision/:id", protect, async (req, res) => {
    try {
        const vision = await PreEmployment.findOne({
            id: req.params.id,
        }).select("id opthalmic_examination");
        res.json(vision);
    } catch (err) {
        console.log(err);
    }
...
router.post("/finalize", protect, async (req, res) => {
    try {
        ...
        if (!preemployment_id || duty_fit === undefined) {
            return res
                .status(400)
                .json({ message: "Missing preemployment_id or duty_fit" });
        }
        ...
        if (!preEmp) {
            return res
                .status(404)
                .json({ message: "Pre-employment record not found" });
        }
